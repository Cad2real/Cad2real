...
def main(hand_type:str, can:str, init_pose=None):
    if init_pose is None:
        init_pose = [255] * 10
    app_ = QApplication(sys.argv)
    window_ = MainWindow(hand_type=hand_type,can=can,init_pos=init_pose)
    window_.show()
...
if __name__ == "__main__":
    ...
    # 0:right 1:left
    LeftOrRight = 1
    init_pos = [200, 0, 196, 188, 0, 0, 0, 255, 255, 235]
    if LeftOrRight:
        window = MainWindow(hand_type="left", can='can1', hand_joint='L10',
                            init_pos=init_pos)
    else:
        window = MainWindow(hand_type="right", can='can0', hand_joint='L10',
                            init_pos=init_pos)
    window.show()
