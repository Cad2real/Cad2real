...
class MainWindow(QMainWindow):
    def __init__(self, hand_type:str, can:str, hand_joint:str, init_pos:List[int]=None):
        super().__init__()
        ...
        self.api = LinkerHandApi(hand_joint=self.hand_joint,hand_type=self.hand_type,can=self.can)
        self.touch_type = -1
        self._init_gui_view()
    ...
    def handle_slider_value_changed(self, slider_values):
        print("实时获取滑动条的当前值:", list(slider_values.values()))
        slider_values_list = []
...
def LinkHand_Gui(hand_type:str, can:str, init_pose=None):
    if init_pose is None:
        init_pose = [255] * 10
    app = QApplication(sys.argv)
    window = MainWindow(hand_type=hand_type,can=can,init_pos=init_pose)
    window.show()
...
if __name__ == "__main__":
    app = QApplication(sys.argv)
    window = MainWindow(hand_type="left", can='can0', hand_joint="L10",
                        init_pos=[200, 0, 200, 167, 0, 0, 0, 0, 0, 240])
    window.show()
