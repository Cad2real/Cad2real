...
from PyQt5.QtWidgets import QWidget, QVBoxLayout, QHBoxLayout, QGridLayout, QLabel, QDockWidget
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        # 创建独立的热力图仪表盘窗口（只创建一次）
        self.dashboard = HeatmapDashboard(n=5, rows=12, cols=8, title="实时触觉热力图 (5路)")
        self.dashboard.setGeometry(100, 100, 1600, 450)  # 窗口大小
        self.dashboard.show()

        # 启动热力图刷新定时器
        self.refresh_ms = 50    # ms
        self.tactile_timer = QTimer(self)
        self.tactile_timer.timeout.connect(self.update_all_heatmaps)
        self.tactile_timer.start(self.refresh_ms)

        # ===== 记录 tactile 数据（CSV/Excel 可直接打开）=====
        self.is_logging = False
        self.log_fp = None
        self.log_writer = None
        self.log_path = None

        self.start_tactile_log()
    ...
    def closeEvent(self, event):
        """关闭窗口时停止线程并释放资源"""
        w = getattr(self, "normal_force_plot", None)
        if w is not None:
            w.close()
        w = getattr(self, "approach_inc_plot", None)
        if w is not None:
            w.close()

        # 关闭热力图 dashboard（如果存在）
        dash = getattr(self, "dashboard", None)
        if dash is not None:
            dash.close()

        self.stop_tactile_log()
        event.accept()

    def update_tactile_heatmap(self):
    
        vec = tactile_stream.get_latest_vector("COM4")  # 你想看哪个口就写哪个
        if vec is None:
            return
        self.tactile_heatmap.update_matrix(vec)
        
    def update_all_heatmaps(self):
        frames = tactile_stream.get_latest_all_vectors(n=5)  # 一次快照 :contentReference[oaicite:2]{index=2}
        self.dashboard.update_all(frames)

        # ===== 记录到 CSV（Excel 可直接打开）=====
        if self.is_logging and self.log_writer is not None:
            # 用毫秒时间戳（本机）
            t_ms = int(time.time() * 1000)
            row = [t_ms]
            for ch in range(5):
                vec = frames[ch] if ch < len(frames) else None
                if vec is None or len(vec) != 96:
                    row.extend([None] * 96)
                else:
                    row.extend(vec)  # 这里写的是“原始 96 点顺序”
            self.log_writer.writerow(row)

            # 可选：每隔一段时间刷盘，避免频繁 flush 卡顿
            # 例如每 50 行 flush 一次（你 refresh_ms=20ms，大约 1 秒）
            if (t_ms // 1000) % 1 == 0:
                self.log_fp.flush()


    def on_refresh_ms_changed(self, ms: int):
        self.refresh_ms = ms
        self.tactile_timer.setInterval(ms)

    def start_tactile_log(self, path=None):
        # If already started, don't reopen
        if self.log_writer is not None:
            return

        base_dir = os.path.join(os.path.expanduser("~"), "Linker_hand_logs")
        os.makedirs(base_dir, exist_ok=True)

        if path is None:
            ts = datetime.now().strftime("%Y%m%d_%H%M%S")
            path = os.path.join(base_dir, f"tactile_log_{ts}.csv")

        try:
            self.log_fp = open(path, "w", newline="", encoding="utf-8")
        except OSError as e:
            print(f"[LOG] cannot open file: {path}\n  -> {e}")
            self.log_fp = None
            self.log_writer = None
            self.is_logging = False
            return
        ...
        # header: timestamp + 5 channels * 96 points
        header = ["timestamp_ms"]
        for ch in range(5):
            for k in range(96):
                header.append(f"ch{ch}_p{k}")
        self.log_writer.writerow(header)
        self.log_fp.flush()

        self.is_logging = True
        print(f"[LOG] start -> {self.log_path}")


    def stop_tactile_log(self):
        ...
        try:
            ...
            self.log_fp = None
            self.log_writer = None
        print(f"[LOG] stop -> {self.log_path}")




class HeatmapWidget(QWidget):
    def __init__(self, rows=12, cols=8, title="Tactile"):
        super().__init__()
        self.rows = rows
        self.cols = cols
        ...
        # Tactile热力图底色
        self.view.setBackground((245, 245, 245))    # 画布背景
        self.vb.setBackgroundColor((245, 245, 245))
        ...
        # h.addWidget(self.lut)

        layout.addWidget(row, 1)

    def update_vector(self, vec_96):
        mat = np.array(vec_96, dtype=np.float32).reshape(self.rows, self.cols)
...
class HeatmapDashboard(QWidget):
    def __init__(self, n=5, rows=12, cols=8, title="Tactile Heatmaps"):
        super().__init__()
        ...
        # 布局：1×5（你也可以改成 2×3）
        for i in range(n):
            hm = HeatmapWidget(rows=rows, cols=cols, title=f"Tactile {i}")
            self.heatmaps.append(hm)
            grid.addWidget(hm, 0, i)

        layout.addLayout(grid)

    def update_all(self, frames):
        """
        frames: List[Optional[List[int]]]，每个元素长度96
        """
        for i, hm in enumerate(self.heatmaps):
            vec = frames[i] if i < len(frames) else None
            if vec is None or len(vec) != 96:
                continue
            hm.update_vector(vec)
