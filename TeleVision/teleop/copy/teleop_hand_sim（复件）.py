...
class VuerTeleop:
    def __init__(self, config_file_path):
        ...
        # load URDF
        RetargetingConfig.set_default_urdf_dir('/home/b600/TeleVision/assets')
    ...
    def step(self):
        ...
        return head_rmat, left_pose, right_pose, left_qpos, right_qpos, right_pose_w
...
class Sim:
    def __init__(self,
        ...
        table_asset = self.gym.create_box(self.sim, 0.8, 1.2, 0.1, table_asset_options)
        ...
        pose.p = gymapi.Vec3(0.35, 0.15, -0.05)
        ...
        # left_hand
        pose = gymapi.Transform()
        pose.p = gymapi.Vec3(0.2, 0.15, 0.55)
        pose.r = gymapi.Quat(0, 0, 0, 1)
...
if __name__ == '__main__':
    ...
    try:
        while True:
            head_rmat, left_pose, right_pose, left_qpos, right_qpos, right_pose_w = teleoperator.step()
            ...
            if i >= key and start:
                ...
                if last_right_pose_w is None or not np.allclose(right_pose_w, last_right_pose_w, atol=1e-6):
                    ...
                    print("T_D_C:")
                    print(', '.join([f'{x:.8f}' for x in T_D_C_result]))

                    target_pose = T_D_C_result
                    new_joint_deg = motion_generator.plan_motion(last_joint_deg, target_pose)
                    if new_joint_deg is not None and len(new_joint_deg) > 0:
                        last_joint_deg = new_joint_deg
                        print("joint_angle:")
                        print("[", ", ".join(f"{x:.2f}" for x in new_joint_deg), "]")
                    else:
                        print("警告: 当前位姿无IK解,已跳过")
