...
class VuerTeleop:
    ...
    def step(self):
        ...
        return head_rmat, left_pose, right_pose, left_qpos, right_qpos, right_landmarks
...
if __name__ == '__main__':
    ...
    simulator = Sim()
    head_file = open('head.txt', 'w')
    right_file = open('right.txt', 'w')
    left_file = open('left.txt', 'w')
    # simulator = Sim()
    key = 50
    i = 0
    t = 1000
    ...
    try:
        while True:
            head_rmat, left_pose, right_pose, left_qpos, right_qpos, right_landmarks = teleoperator.step()
            ...
            if i >= key and start:
                ...
                right_angles = hand_retarget.solve_fingers_angles(right_landmarks)
                print('right_angles:\n',right_angles)
                print('This is the '+str(write)+'\n')
                write += 1
                head_file.write(str(head_rmat))
                head_file.write('\n')
                right_file.write(" ".join(f"{num:.8f}" for num in right_pose) + "\n")
                left_file.write(" ".join(f"{num:.8f}" for num in left_pose) + "\n")
